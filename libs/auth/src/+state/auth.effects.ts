...
export class AuthEffects {
  ...
  setLocalStorage = this.actions
    .ofType<SetLocalStorage>('[auth] SET_LOCAL_STORAGE')
    .pipe(
      map(action => action.payload),
      tap(token => this.localStorageJwtService.setItem(token)),
      concatMap(_ => [{ type: '[auth] GET_USER' }, { type: '[Router] Go', payload: { path: ['/'] } }])
    );

  @Effect()
  removeoLcalStorage = this.actions
    .ofType<SetLocalStorage>('[auth] REMOVE_LOCAL_STORAGE')
    .pipe(
      map(action => action.payload),
      tap(token => this.localStorageJwtService.removeItem()),
      concatMap(_ => [{ type: '[auth] INITIALIZE_USER' }, { type: '[Router] Go', payload: { path: ['/'] } }])
    );
